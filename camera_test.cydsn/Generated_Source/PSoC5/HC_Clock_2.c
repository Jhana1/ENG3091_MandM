...
/*******************************************************************************
* Function Name: HC_Clock_2_Start
********************************************************************************
*
* Summary:
...
void HC_Clock_2_Start(void) 
{
    ...
    HC_Clock_2_CLKEN |= HC_Clock_2_CLKEN_MASK;
	HC_Clock_2_CLKSTBY |= HC_Clock_2_CLKSTBY_MASK;
}


/*******************************************************************************
* Function Name: HC_Clock_2_Stop
********************************************************************************
*
* Summary:
...
void HC_Clock_2_Stop(void) 
{
    ...
    HC_Clock_2_CLKEN &= (uint8)(~HC_Clock_2_CLKEN_MASK);
	HC_Clock_2_CLKSTBY &= (uint8)(~HC_Clock_2_CLKSTBY_MASK);
}
...
/*******************************************************************************
* Function Name: HC_Clock_2_StopBlock
********************************************************************************
*
* Summary:
...
{
    if ((HC_Clock_2_CLKEN & HC_Clock_2_CLKEN_MASK) != 0u)
    {
        ...
        CLK_DIST_AMASK = HC_Clock_2_CLKEN_MASK;
        CLK_DIST_DMASK = 0x00u;
#else
        CLK_DIST_DMASK = HC_Clock_2_CLKEN_MASK;
        CLK_DIST_AMASK = 0x00u;
#endif /* HC_Clock_2__CFG3 */

        /* Clear mask of bus clock. */
        CLK_DIST_BCFG2 &= (uint8)(~BCFG2_MASK);

        oldDivider = CY_GET_REG16(HC_Clock_2_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(HC_Clock_2_DIV_PTR, oldDivider);
#endif /* HAS_CLKDIST_LD_DISABLE */
    }
}
...
/*******************************************************************************
* Function Name: HC_Clock_2_StandbyPower
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: HC_Clock_2_SetDividerRegister
********************************************************************************
*
* Summary:
                                ...
                                
{
    ...
    uint8 currSrc = HC_Clock_2_GetSourceRegister();
    uint16 oldDivider = HC_Clock_2_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = HC_Clock_2_CLKEN & HC_Clock_2_CLKEN_MASK;
        ...
        else
        {
            ...
            if (enabled != 0u)
            {
                ...
                if (((HC_Clock_2_MOD_SRC & CYCLK_SYNC) == 0u) || (clkDivider == 0u))
                {
#if HAS_CLKDIST_LD_DISABLE
                    CY_SET_REG16(CYREG_CLKDIST_WRK0, oldDivider);
                    CLK_DIST_LD = CYCLK_LD_DISABLE|CYCLK_LD_SYNC_EN|CYCLK_LD_LOAD;
...
/*******************************************************************************
* Function Name: HC_Clock_2_GetDividerRegister
********************************************************************************
*
* Summary:
...
{
    return CY_GET_REG16(HC_Clock_2_DIV_PTR);
}


/*******************************************************************************
* Function Name: HC_Clock_2_SetModeRegister
********************************************************************************
*
* Summary:
...
{
    HC_Clock_2_MOD_SRC |= modeBitMask & (uint8)HC_Clock_2_MODE_MASK;
}


/*******************************************************************************
* Function Name: HC_Clock_2_ClearModeRegister
********************************************************************************
*
* Summary:
...
{
    HC_Clock_2_MOD_SRC &= (uint8)(~modeBitMask) | (uint8)(~(uint8)(HC_Clock_2_MODE_MASK));
}


/*******************************************************************************
* Function Name: HC_Clock_2_GetModeRegister
********************************************************************************
*
* Summary:
...
{
    return HC_Clock_2_MOD_SRC & (uint8)(HC_Clock_2_MODE_MASK);
}


/*******************************************************************************
* Function Name: HC_Clock_2_SetSourceRegister
********************************************************************************
*
* Summary:
...
{
    uint16 currDiv = HC_Clock_2_GetDividerRegister();
    uint8 oldSrc = HC_Clock_2_GetSourceRegister();
        ...
        (clkSource == ((uint8)CYCLK_SRC_SEL_CLK_SYNC_D))) && (currDiv == 0u))
    {
        ...
        HC_Clock_2_MOD_SRC |= CYCLK_SSS;
        HC_Clock_2_MOD_SRC =
            (HC_Clock_2_MOD_SRC & (uint8)(~HC_Clock_2_SRC_SEL_MSK)) | clkSource;
    }
...
/*******************************************************************************
* Function Name: HC_Clock_2_GetSourceRegister
********************************************************************************
*
* Summary:
...
{
    return HC_Clock_2_MOD_SRC & HC_Clock_2_SRC_SEL_MSK;
}
...
/*******************************************************************************
* Function Name: HC_Clock_2_SetPhaseRegister
********************************************************************************
*
* Summary:
...
{
    HC_Clock_2_PHASE = clkPhase & HC_Clock_2_PHASE_MASK;
}


/*******************************************************************************
* Function Name: HC_Clock_2_GetPhase
********************************************************************************
*
* Summary:
...
{
    return HC_Clock_2_PHASE & HC_Clock_2_PHASE_MASK;
}
