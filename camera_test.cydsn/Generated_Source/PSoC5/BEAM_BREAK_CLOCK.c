...
/*******************************************************************************
* Function Name: BEAM_BREAK_CLOCK_Start
********************************************************************************
*
* Summary:
...
void BEAM_BREAK_CLOCK_Start(void) 
{
    ...
    BEAM_BREAK_CLOCK_CLKEN |= BEAM_BREAK_CLOCK_CLKEN_MASK;
	BEAM_BREAK_CLOCK_CLKSTBY |= BEAM_BREAK_CLOCK_CLKSTBY_MASK;
}


/*******************************************************************************
* Function Name: BEAM_BREAK_CLOCK_Stop
********************************************************************************
*
* Summary:
...
void BEAM_BREAK_CLOCK_Stop(void) 
{
    ...
    BEAM_BREAK_CLOCK_CLKEN &= (uint8)(~BEAM_BREAK_CLOCK_CLKEN_MASK);
	BEAM_BREAK_CLOCK_CLKSTBY &= (uint8)(~BEAM_BREAK_CLOCK_CLKSTBY_MASK);
}
...
/*******************************************************************************
* Function Name: BEAM_BREAK_CLOCK_StopBlock
********************************************************************************
*
* Summary:
...
{
    if ((BEAM_BREAK_CLOCK_CLKEN & BEAM_BREAK_CLOCK_CLKEN_MASK) != 0u)
    {
        ...
        CLK_DIST_AMASK = BEAM_BREAK_CLOCK_CLKEN_MASK;
        CLK_DIST_DMASK = 0x00u;
#else
        CLK_DIST_DMASK = BEAM_BREAK_CLOCK_CLKEN_MASK;
        CLK_DIST_AMASK = 0x00u;
#endif /* BEAM_BREAK_CLOCK__CFG3 */

        /* Clear mask of bus clock. */
        CLK_DIST_BCFG2 &= (uint8)(~BCFG2_MASK);

        oldDivider = CY_GET_REG16(BEAM_BREAK_CLOCK_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(BEAM_BREAK_CLOCK_DIV_PTR, oldDivider);
#endif /* HAS_CLKDIST_LD_DISABLE */
    }
}
...
/*******************************************************************************
* Function Name: BEAM_BREAK_CLOCK_StandbyPower
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: BEAM_BREAK_CLOCK_SetDividerRegister
********************************************************************************
*
* Summary:
                                ...
                                
{
    ...
    uint8 currSrc = BEAM_BREAK_CLOCK_GetSourceRegister();
    uint16 oldDivider = BEAM_BREAK_CLOCK_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = BEAM_BREAK_CLOCK_CLKEN & BEAM_BREAK_CLOCK_CLKEN_MASK;
        ...
        if ((currSrc == (uint8)CYCLK_SRC_SEL_CLK_SYNC_D) && ((oldDivider == 0u) || (clkDivider == 0u)))
        {
            ...
            if (oldDivider == 0u)
            {
                ...
                CY_SET_REG16(BEAM_BREAK_CLOCK_DIV_PTR, clkDivider);
                BEAM_BREAK_CLOCK_MOD_SRC &= (uint8)(~CYCLK_SSS);
            }
            else
            {
                /* Moving to SSS, set SSS which then ignores the divider and we can set     */
                /* it without bothering with the shadow load.                               */
                BEAM_BREAK_CLOCK_MOD_SRC |= CYCLK_SSS;
                CY_SET_REG16(BEAM_BREAK_CLOCK_DIV_PTR, clkDivider);
            }
        }
        else
        {
            ...
            if (enabled != 0u)
            {
                ...
                if (((BEAM_BREAK_CLOCK_MOD_SRC & CYCLK_SYNC) == 0u) || (clkDivider == 0u))
                {
#if HAS_CLKDIST_LD_DISABLE
                    CY_SET_REG16(CYREG_CLKDIST_WRK0, oldDivider);
                    CLK_DIST_LD = CYCLK_LD_DISABLE|CYCLK_LD_SYNC_EN|CYCLK_LD_LOAD;
            ...
            else
            {
                ...
                CY_SET_REG16(BEAM_BREAK_CLOCK_DIV_PTR, clkDivider);
				BEAM_BREAK_CLOCK_CLKEN |= enabled;
            }
        }
    }
}
...
/*******************************************************************************
* Function Name: BEAM_BREAK_CLOCK_GetDividerRegister
********************************************************************************
*
* Summary:
...
{
    return CY_GET_REG16(BEAM_BREAK_CLOCK_DIV_PTR);
}


/*******************************************************************************
* Function Name: BEAM_BREAK_CLOCK_SetModeRegister
********************************************************************************
*
* Summary:
...
{
    BEAM_BREAK_CLOCK_MOD_SRC |= modeBitMask & (uint8)BEAM_BREAK_CLOCK_MODE_MASK;
}


/*******************************************************************************
* Function Name: BEAM_BREAK_CLOCK_ClearModeRegister
********************************************************************************
*
* Summary:
...
{
    BEAM_BREAK_CLOCK_MOD_SRC &= (uint8)(~modeBitMask) | (uint8)(~(uint8)(BEAM_BREAK_CLOCK_MODE_MASK));
}


/*******************************************************************************
* Function Name: BEAM_BREAK_CLOCK_GetModeRegister
********************************************************************************
*
* Summary:
...
{
    return BEAM_BREAK_CLOCK_MOD_SRC & (uint8)(BEAM_BREAK_CLOCK_MODE_MASK);
}


/*******************************************************************************
* Function Name: BEAM_BREAK_CLOCK_SetSourceRegister
********************************************************************************
*
* Summary:
...
{
    uint16 currDiv = BEAM_BREAK_CLOCK_GetDividerRegister();
    uint8 oldSrc = BEAM_BREAK_CLOCK_GetSourceRegister();
        ...
        (clkSource == ((uint8)CYCLK_SRC_SEL_CLK_SYNC_D))) && (currDiv == 0u))
    {
        ...
        BEAM_BREAK_CLOCK_MOD_SRC |= CYCLK_SSS;
        BEAM_BREAK_CLOCK_MOD_SRC =
            (BEAM_BREAK_CLOCK_MOD_SRC & (uint8)(~BEAM_BREAK_CLOCK_SRC_SEL_MSK)) | clkSource;
    }
...
/*******************************************************************************
* Function Name: BEAM_BREAK_CLOCK_GetSourceRegister
********************************************************************************
*
* Summary:
...
{
    return BEAM_BREAK_CLOCK_MOD_SRC & BEAM_BREAK_CLOCK_SRC_SEL_MSK;
}
...
/*******************************************************************************
* Function Name: BEAM_BREAK_CLOCK_SetPhaseRegister
********************************************************************************
*
* Summary:
...
{
    BEAM_BREAK_CLOCK_PHASE = clkPhase & BEAM_BREAK_CLOCK_PHASE_MASK;
}


/*******************************************************************************
* Function Name: BEAM_BREAK_CLOCK_GetPhase
********************************************************************************
*
* Summary:
...
{
    return BEAM_BREAK_CLOCK_PHASE & BEAM_BREAK_CLOCK_PHASE_MASK;
}
