/*******************************************************************************
* File Name: OV9650_1_I2C_SLAVE.c
* Version 3.50
*
* Description:
...
volatile uint8 OV9650_1_I2C_slStatus;   /* Slave Status  */

/* Transmit buffer variables */
volatile uint8 * OV9650_1_I2C_slRdBufPtr;   /* Pointer to Transmit buffer  */
volatile uint8   OV9650_1_I2C_slRdBufSize;  /* Slave Transmit buffer size  */
volatile uint8   OV9650_1_I2C_slRdBufIndex; /* Slave Transmit buffer Index */

/* Receive buffer variables */
volatile uint8 * OV9650_1_I2C_slWrBufPtr;   /* Pointer to Receive buffer  */
volatile uint8   OV9650_1_I2C_slWrBufSize;  /* Slave Receive buffer size  */
volatile uint8   OV9650_1_I2C_slWrBufIndex; /* Slave Receive buffer Index */

#if (OV9650_1_I2C_SW_ADRR_DECODE)
    volatile uint8 OV9650_1_I2C_slAddress;  /* Software address variable */
#endif /* (OV9650_1_I2C_SW_ADRR_DECODE) */


/*******************************************************************************
* Function Name: OV9650_1_I2C_SlaveStatus
********************************************************************************
*
* Summary:
...
*
* Global variables:
*  OV9650_1_I2C_slStatus - The global variable used to store a current
*                              status of the I2C slave.
*
*******************************************************************************/
uint8 OV9650_1_I2C_SlaveStatus(void) 
{
    return (OV9650_1_I2C_slStatus);
}


/*******************************************************************************
* Function Name: OV9650_1_I2C_SlaveClearReadStatus
********************************************************************************
*
* Summary:
*  Clears the read status flags and returns they values.
*  The OV9650_1_I2C_SSTAT_RD_BUSY flag is not effected by clear.
*
* Parameters:
*  None.
*
...
uint8 OV9650_1_I2C_SlaveClearReadStatus(void) 
{
    ...
    status = (OV9650_1_I2C_slStatus & OV9650_1_I2C_SSTAT_RD_MASK);
    OV9650_1_I2C_slStatus &= (uint8) ~OV9650_1_I2C_SSTAT_RD_CLEAR;
...
/*******************************************************************************
* Function Name: OV9650_1_I2C_SlaveClearWriteStatus
********************************************************************************
*
* Summary:
*  Clears the write status flags and returns they values.
*  The OV9650_1_I2C_SSTAT_WR_BUSY flag is not effected by clear.
*
* Parameters:
*  None.
*
...
uint8 OV9650_1_I2C_SlaveClearWriteStatus(void) 
{
    ...
    /* Mask of transfer complete flag and Error status */
    status = (OV9650_1_I2C_slStatus & OV9650_1_I2C_SSTAT_WR_MASK);
    OV9650_1_I2C_slStatus &= (uint8) ~OV9650_1_I2C_SSTAT_WR_CLEAR;
...
/*******************************************************************************
* Function Name: OV9650_1_I2C_SlaveSetAddress
********************************************************************************
*
* Summary:
...
*
* Global variables:
*  OV9650_1_I2C_Address  - The global variable used to store an I2C slave
...
{
#if (OV9650_1_I2C_SW_ADRR_DECODE)
    OV9650_1_I2C_slAddress = (address & OV9650_1_I2C_SLAVE_ADDR_MASK);
#else
    OV9650_1_I2C_ADDR_REG  = (address & OV9650_1_I2C_SLAVE_ADDR_MASK);
#endif /* (OV9650_1_I2C_SW_ADRR_DECODE) */
}


/*******************************************************************************
* Function Name: OV9650_1_I2C_SlaveInitReadBuf
********************************************************************************
*
* Summary:
...
*
* Global variables:
*  OV9650_1_I2C_slRdBufPtr   - The global variable used to store a pointer
*                                  to the slave read buffer.
*  OV9650_1_I2C_slRdBufSize  - The global variable used to store a slave
*                                  read buffer size.
*  OV9650_1_I2C_slRdBufIndex - The global variable used to store a current
     ...
     
{
    if (NULL != rdBuf)
    {
        ...
        /* Set buffer pointer */
        OV9650_1_I2C_slRdBufPtr   = (volatile uint8 *) rdBuf;
        OV9650_1_I2C_slRdBufSize  = bufSize;    /* Set buffer size     */
        OV9650_1_I2C_slRdBufIndex = 0u;         /* Clears buffer index */
...
/*******************************************************************************
* Function Name: OV9650_1_I2C_SlaveInitWriteBuf
********************************************************************************
*
* Summary:
*  Sets the buffer pointer and size of the read buffer. This function also
...
*
* Global variables:
*  OV9650_1_I2C_slWrBufPtr   - The global variable used to store a pointer
*                                  to the slave write buffer.
*  OV9650_1_I2C_slWrBufSize  - The global variable used to store a slave
*                                  write buffer size.
*  OV9650_1_I2C_slWrBufIndex - The global variable used to store a current
     ...
     
{
    if (NULL != wrBuf)
    {
        OV9650_1_I2C_DisableInt(); /* Lock from interrupt */
         ...
         /* Set buffer pointer */
        OV9650_1_I2C_slWrBufPtr   = (volatile uint8 *) wrBuf;
        OV9650_1_I2C_slWrBufSize  = bufSize;    /* Set buffer size     */
        OV9650_1_I2C_slWrBufIndex = 0u;         /* Clears buffer index */
...
/*******************************************************************************
* Function Name: OV9650_1_I2C_SlaveGetReadBufSize
********************************************************************************
*
* Summary:
...
{
    return (OV9650_1_I2C_slRdBufIndex);
}


/*******************************************************************************
* Function Name: OV9650_1_I2C_SlaveGetWriteBufSize
********************************************************************************
*
* Summary:
...
{
    return (OV9650_1_I2C_slWrBufIndex);
}


/*******************************************************************************
* Function Name: OV9650_1_I2C_SlaveClearReadBuf
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: OV9650_1_I2C_SlaveClearRxBuf
********************************************************************************
*
* Summary:
