...
static uint8  mSerial_started = 0u;


/*******************************************************************************
* Function Name: mSerial_CyBtldrCommStart
********************************************************************************
*
* Summary:
...
void mSerial_CyBtldrCommStart(void) 
{
    ...
    mSerial_Start(0u, mSerial_DWR_VDDD_OPERATION);
...
/*******************************************************************************
* Function Name: mSerial_CyBtldrCommStop.
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: mSerial_CyBtldrCommReset.
********************************************************************************
*
* Summary:
...
{
    mSerial_EnableOutEP(mSerial_BTLDR_OUT_EP);  /* Enable the OUT endpoint */
}


/*******************************************************************************
* Function Name: mSerial_CyBtldrCommWrite.
********************************************************************************
*
* Summary:
                                                            ...
                                                            
{
    ...
    /* Enable IN transfer */
    mSerial_LoadInEP(mSerial_BTLDR_IN_EP, pData, mSerial_BTLDR_SIZEOF_READ_BUFFER);

    /* Wait for the master to read it. */
    while ((mSerial_GetEPState(mSerial_BTLDR_IN_EP) == mSerial_IN_BUFFER_FULL) &&
           (0u != timeoutMs))
    {
        CyDelay(mSerial_BTLDR_WAIT_1_MS);
        timeoutMs--;
    }

    if (mSerial_GetEPState(mSerial_BTLDR_IN_EP) == mSerial_IN_BUFFER_FULL)
    {
...
/*******************************************************************************
* Function Name: mSerial_CyBtldrCommRead.
********************************************************************************
*
* Summary:
                                                            ...
                                                            
{
    ...
    if (size > mSerial_BTLDR_SIZEOF_WRITE_BUFFER)
    {
        size = mSerial_BTLDR_SIZEOF_WRITE_BUFFER;
    }

    /* Wait on enumeration in first time */
    if (0u != mSerial_started)
    {
        /* Wait for Device to enumerate */
        while ((0u ==mSerial_GetConfiguration()) && (0u != timeoutMs))
        {
            CyDelay(mSerial_BTLDR_WAIT_1_MS);
            timeoutMs--;
        }
        ...
        if (0u != mSerial_GetConfiguration())
        {
            (void) mSerial_IsConfigurationChanged();  /* Clear configuration changes state status */
            mSerial_CyBtldrCommReset();
            mSerial_started = 0u;
        }
    }
    else /* Check for configuration changes, has been done by Host */
    {
        if (0u != mSerial_IsConfigurationChanged()) /* Host could send double SET_INTERFACE request or RESET */
        {
            if (0u != mSerial_GetConfiguration())   /* Init OUT endpoints when device reconfigured */
            {
                mSerial_CyBtldrCommReset();
            }
        }
    ...
    /* Wait on next packet */
    while((mSerial_GetEPState(mSerial_BTLDR_OUT_EP) != mSerial_OUT_BUFFER_FULL) && \
          (0u != timeoutMs))
    {
        CyDelay(mSerial_BTLDR_WAIT_1_MS);
        timeoutMs--;
    ...
    if (mSerial_GetEPState(mSerial_BTLDR_OUT_EP) == mSerial_OUT_BUFFER_FULL)
    {
        *count = mSerial_ReadOutEP(mSerial_BTLDR_OUT_EP, pData, size);
        retCode = CYRET_SUCCESS;
    }
    else
    {
...
#endif /*  CYDEV_BOOTLOADER_IO_COMP == CyBtldr_mSerial */
